...
export async function openConversationStore(idb = globalThis.indexedDB, dbName = HISTORY_DB) {
...
export function memoryConversationStore(seed, key = 'tax-law-chat.memory.v2') {
  let rows = seed ? seed.map(row => cleanConversation(row, true)) : [];
  if (!seed) try { rows = JSON.parse(sessionStorage.getItem(key) || '[]').map(row => cleanConversation(row, true)); } catch {}
  const persist = () => { try { sessionStorage.setItem(key, JSON.stringify(rows)); } catch {} };
